...
TEST(QuickFAST, testRoundTripFieldOperators)
{
  ...
  EXPECT_EQ((templateRegistry->maxFieldCount()), (14));
  ...
  EXPECT_TRUE(fast1A != fast1B);
  EXPECT_TRUE(fast1B == fast1C);
  EXPECT_TRUE(fast2A != fast2B);


  EXPECT_TRUE(compareMessages(*msg1, consumer1A.message()));
  EXPECT_TRUE(compareMessages(consumer1A.message(), consumer1B.message()));
  EXPECT_TRUE(compareMessages(consumer1A.message(), consumer1C.message()));
  EXPECT_TRUE(compareMessages(consumer2A.message(), consumer2B.message()));
}

TEST(QuickFAST, testRoundTripOptionalTemplateRefs)
{
  Codecs::XMLTemplateParser parser;
  std::stringstream templateStream(theTemplates);
  Codecs::TemplateRegistryPtr templateRegistry =
  ...
  SCOPED_TRACE("encode addMessage");
  ...
  SCOPED_TRACE("encode delMessage");
  ...
  SCOPED_TRACE("decode addMessage");
  ...
  EXPECT_TRUE(compareMessages(*addFlatMessage, consumerAdd.message()));
  SCOPED_TRACE("decode delMessage");
  ...
  EXPECT_TRUE(compareMessages(*delFlatMessage, consumerDel.message()));
}
